...
function TestFunctionConfig (testClass, testfunc, parameters, expectError, prerunfuncs) {
    ...
    this.testfunc = testfunc || null // the test function name
...
function test(config, callback, prerunfunc) {
    ...
    testClass[testfunc](...parameters, error => {
        if (!error) {
            if (expectError) {
                console.log(chalk.red((prerunfunc ? "Test prerunfunc:" : '') + testfunc + " expect " + getErrorNameByCode(expectError) + ' failed due to'), 'received no error')
                callback(false, testfunc + " expect " + getErrorNameByCode(expectError) + ' failed due to received no error')
            } else {
                if (!prerunfunc) {
                    console.log(chalk.green((prerunfunc ? "Test prerunfunc:" : '') + testfunc + ' passed'))
                }
                callback(true)
            }
        } else {
            if (expectError) {
                if (expectError === error.Status) {
                    ...
                    console.log(chalk.red((prerunfunc ? "Test prerunfunc:" : '') + testfunc + ' expect ' + getErrorNameByCode(expectError) + ' failed due to'), 'received other error', error)
                    callback(false, (prerunfunc ? "Test prerunfunc:" : '') + testfunc + ' expect ' + getErrorNameByCode(expectError) + ' failed due to received other error' + JSON.stringify(error))
                }
            } else {
